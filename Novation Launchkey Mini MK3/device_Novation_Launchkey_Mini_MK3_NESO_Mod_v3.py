#   name=Novation Launchkey Mini MK3 (NESO Mod) v3.0
...
from timehandling import buzzergen
#import schedmap
apiver4script=20
...
def OnInit():           # Set up "DAW mode" in the controller after linking the script to the controller. RTFM to know why is this important!
    if general.getVersion() >= apiver4script:
        ...
        print("")
        print("Attempting to start anyways...")
        initrfsh.Initialize()
...
def OnIdle():
    ...
    #print(var.currenttime)
    #Shortcuts.Repeater
    #Repeater(presstime, 1)
    """if bool(var.repeatqueue):
        for x,y in var.repeatqueue.items():
            if y:
                if var.currenttime - y > 0.3:
                    var.repeatok[x] = True
                    print(buzzergen(1))
                    #schedmap.Trigger()
    """
